layui.config({
    ...
    table.render({
        ...
        cols: [[
            ...
            { title: '操作', fixed: 'right', align: 'center', toolbar: '#barPower', width: 120 }
        ]]
    });
    //监听工具条
    table.on('tool(powertable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'del') { //删除
            ...
            $.ajax({
                type: 'get',
                url: 'http://www.crm.com/powerEdit?id=' + data.id,
                async: true,
    ...
    function deleteRole(ids, obj) {
        var msg = obj ? '确认删除权限【' + obj.data.roleName + '】吗？' : '确认删除选中数据吗？'
        top.winui.window.confirm(msg, { icon: 3, title: '删除系统权限' }, function (index) {
            layer.close(index);
            //向服务端发送删除指令
            //刷新表格
            if (obj) {
